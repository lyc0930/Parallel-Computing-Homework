...
\paragraph{解}
\subparagraph{(1)}
注意到第\ref{BigLoop}行最外侧的循环迭代$\lceil \log{n} \rceil$次，在循环内第\ref{SmallLoop}行循环同样迭代$\lceil \log{n} \rceil$次，同时考虑外侧循环内的前两个Do in parallel循环语句中的求出最小值也是并行的，需要的处理器数为$(n - 1)^2$，因此算法的复杂度为$t(n) = (\log{n})^2$，$p(n) = n^2$。
